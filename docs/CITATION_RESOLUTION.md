...
- performs exact lookup for other preserved identifier schemes, whose ingestion adapters are responsible for supplying canonical values until Tarkka defines a shared scheme normalizer;
...
Repeated resolution reuses equivalent resolution and relation records rather than creating duplicate graph edges. Relation reuse is keyed directly by deterministic relation ID, avoiding scans over all outgoing citations.

## Persistence and concurrency

Resolution identity is content-based: `resolved_at` records when the first equivalent result was persisted and is intentionally excluded from idempotency comparison. Repository implementations must serialize writes to a reference's resolution key. The local JSON repository uses its exclusive file lock; a PostgreSQL adapter should enforce the same invariant with a unique constraint plus transactional `INSERT ... ON CONFLICT`/locking semantics.
