...
| --- | --- | --- | --- | --- | --- | --- |
| Research document repository | `src/tarkka/infrastructure/storage/json_repository.py`; PostgreSQL `research_repository.py` and migrations through `0014` | JSON and PostgreSQL contract suites, including preserved table-cell round trips | contract suite plus native-ingest/vertical-slice coverage | JSON schema-corruption regressions; PostgreSQL checks constrain cell coordinates, spans, role, text, and anchors | `tests/test_ingest_persistence_recovery.py`; PostgreSQL writes occur within the document transaction | **Covered** |
...
| PostgreSQL evidence relations | `src/tarkka/infrastructure/postgres/verification_repository.py`, migration `0009` | unit decoding plus `tests/test_postgres_verification_repository_contract.py` against an ephemeral migrated PostgreSQL service | immutable deterministic IDs, idempotent re-save, incompatible-content rejection, and bounded reads | PostgreSQL schema checks enforce Claim/context lineage, exact-evidence shape, and typed values | real PostgreSQL repository contract covers write/read behavior and immutable-row rejection; driver errors are translated at the adapter boundary | **Covered** for the current repository port; interface backend selection remains intentionally deferred until extraction/citation ports are also PostgreSQL-backed |
