...
The resulting canonical `Document` represents the linear EPUB reading order. IDs are deterministic from the immutable EPUB artifact plus package/spine identity, and passage character offsets are rebased into one document-wide coordinate space.
...
XHTML spine items are parsed through controlled temporary files only. Archive member paths are never extracted directly onto the filesystem.
...
- rejects absolute paths, parent-directory traversal, and backslash archive paths;
...
The first EPUB adapter intentionally supports linear `application/xhtml+xml` and `text/html` spine content. Non-linear spine entries remain represented in the source-native spine metadata but are not added to canonical reading order.
