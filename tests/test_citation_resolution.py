...
def test_conflicting_exact_identifiers_fail_closed_as_ambiguous() -> None:
    ...
    resolution = _resolver({("doi", "10.1000/example"): doi_work, ("arxiv", "2401.01234"): arxiv_work}).resolve(reference)
...
def test_multiple_exact_identifiers_for_same_work_resolve_once() -> None:
    ...
    resolution = _resolver({("doi", "10.1000/example"): work, ("arxiv", "2401.01234"): work}).resolve(reference)
    assert resolution.status is CitationResolutionStatus.RESOLVED
    assert resolution.work_id == work.work_id
...
def test_resolved_reference_creates_provenance_backed_cites_relation() -> None:
    ...
    relation = resolver.relation_for_resolved_reference(citing_work_id=citing_id, reference=reference, resolution=resolution)
...
def test_self_citation_creates_explicit_cites_relation() -> None:
    ...
    resolution = resolver.resolve(reference)
    relation = resolver.relation_for_resolved_reference(citing_work_id=work.work_id, reference=reference, resolution=resolution)
...
def test_relation_rejects_unresolved_or_mismatched_resolution() -> None:
    ...
    with pytest.raises(ValueError, match="resolved reference"):
        resolver.relation_for_resolved_reference(citing_work_id=uuid4(), reference=reference, resolution=unresolved)
    cited = _work()
    ...
    with pytest.raises(ValueError, match="does not belong"):
        resolver.relation_for_resolved_reference(citing_work_id=uuid4(), reference=reference, resolution=resolved)
