...
def test_claim_receipt_service_uses_one_postgres_settings_object(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    sentinel_settings = object()
    seen: list[tuple[str, object]] = []
    ...
    service = runtime.claim_receipt_service()

    assert service.__class__.__name__ == "ClaimReceiptService"
    assert [name for name, _settings in seen] == [
        "extraction",
        "research",
        "verification",
        "citation",
    ]
    assert {settings for _name, settings in seen} == {sentinel_settings}
