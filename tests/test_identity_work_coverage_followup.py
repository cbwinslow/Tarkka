...
class _UnexpectedBoundaryCall(RuntimeError):
    """A test double was called after the public service should have failed closed."""
...
def test_fuzzy_identity_handles_missing_year_and_conflicting_valid_dois() -> None:
    ...
    assert matcher.compare(
        _record("crossref", "C2", doi="10.1000/fake-doi-1"),
        _record("openalex", "W2", doi="10.1000/fake-doi-2"),
    ) is None


class _ExplodingSnapshots:
    def get(self, snapshot_id: UUID) -> SearchSnapshot | None:
        raise _UnexpectedBoundaryCall(
            f"snapshot lookup must not occur for negative index: {snapshot_id}"
        )
...
class _ExplodingCatalog:
    def persist_candidate(self, candidate: CanonicalWorkCandidate) -> Work:
        del candidate
        raise _UnexpectedBoundaryCall("catalog must not be called for negative index")
...
class _RacingAliasRepository:
    def __init__(self, conflicting_work: Work) -> None:
        ...
        self.saved_identifiers: list[WorkIdentifier] = []
        self.saved_source_records: list[WorkSourceRecord] = []
    ...
    def find_work_by_identifier(self, scheme: str, value: str) -> Work | None:
        self.lookup_keys.append((scheme, value))
        return None if len(self.lookup_keys) == 1 else self.conflicting_work

    def save_identifier(self, identifier: WorkIdentifier) -> None:
        self.saved_identifiers.append(identifier)

    def list_identifiers(self, work_id: UUID) -> tuple[WorkIdentifier, ...]:
        del work_id
        return tuple(self.saved_identifiers)

    def save_source_record(self, source_record: WorkSourceRecord) -> None:
        self.saved_source_records.append(source_record)

    def list_source_records(self, work_id: UUID) -> tuple[WorkSourceRecord, ...]:
        return tuple(
            record for record in self.saved_source_records if record.work_id == work_id
        )
...
def test_work_catalog_fails_closed_on_alias_race() -> None:
    ...
    assert repository.saved_identifiers == []
    assert repository.saved_source_records == []
