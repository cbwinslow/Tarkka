...
def test_why_cli_reads_persisted_local_lineage_end_to_end(
    tmp_path: Path,
    ...
    assert entrypoint.main(["why", f"claim:{claim.extraction_id}"]) == 0
    payload = json.loads(capsys.readouterr().out)

    assert payload["claim"]["claim_id"] == str(claim.extraction_id)
    assert payload["claim"]["text"] == claim.text
    assert payload["claim_source"]["artifact"]["sha256"] == artifact.sha256
    assert payload["claim_evidence"][0]["source_kind"] == "passage"
...
def _lineage_for(evidence: object) -> EvidenceLineage:
    artifact, document, _, _, _, _ = _fixture()
    return EvidenceLineage(
        evidence=evidence,
        source=(
            document.figures[0]
            if isinstance(evidence, FigureEvidence)
            else document.tables[0]
            if isinstance(evidence, TableEvidence)
            else document.equations[0]
            if isinstance(evidence, EquationEvidence)
            else document.sections[0].passages[0]
        ),
        lineage=SourceLineage(document=document, artifact=artifact),
    )


def test_evidence_payload_serializes_every_structural_locator() -> None:
    _, document, run, text_evidence, _, _ = _fixture()
    provenance = ExtractionProvenance(run_id=run.run_id)
    values = (
        text_evidence,
        FigureEvidence(
            evidence_id=_id(11),
    ...
    payloads = [why_cli._evidence_payload(_lineage_for(item)) for item in values]

    assert [item["source_kind"] for item in payloads] == ["passage", "figure", "table", "equation"]
    assert payloads[0]["passage_char_end"] == 5
    assert payloads[1]["figure_id"] == str(document.figures[0].figure_id)
    assert payloads[2]["row_end"] == 1
    assert payloads[3]["equation_id"] == str(document.equations[0].equation_id)
...
def test_assessment_payload_preserves_context_without_treating_it_as_evidence() -> None:
    ...
    with_context = why_cli._assessment_payload(
        ClaimAssessmentLineage(
            relation=relation,
            evidence=_lineage_for(evidence),
...
def test_why_cli_reports_lineage_errors(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    assert why_cli.main([str(_id(999))]) == 2
    assert "claim not found" in capsys.readouterr().err


def test_why_documents_factory_supports_postgres_without_changing_application_contract(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    class _PostgresDocuments:
        def __init__(self, settings: object) -> None:
            seen.append(settings)

    monkeypatch.setattr(why_cli, "_document_backend", lambda: "postgres")
    monkeypatch.setattr(why_cli.PostgresSettings, "from_environment", lambda: sentinel_settings)
    monkeypatch.setattr(why_cli, "PostgresResearchRepository", _PostgresDocuments)

    result = why_cli._documents(tmp_path)

    assert isinstance(result, _PostgresDocuments)
    assert seen == [sentinel_settings]
